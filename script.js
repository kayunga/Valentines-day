...
button.addEventListener("click", () => {
  if (button.textContent === "Click Me! ❤") {
    ...
    fetch('send_mail.php')
      .then(response => {
        if (response.ok) {
          button.textContent = "I FREAKING LOVE U HELLEN 🙃";
        } else {
...
function drawText() {
    ...
    if(frameNumber > 2500 && frameNumber < 99999){
        ...
        if (window.innerWidth < 600) {
            drawTextWithLineBreaks(["I love you so much Priscovia, more than", "all the time and space in the universe can contain"], canvas.width / 2, canvas.height / 2, fontSize, lineHeight);
        } else {
            context.fillText("I love you so much Priscovia, more than all the time and space in the universe can contain", canvas.width/2, canvas.height/2);
        }
